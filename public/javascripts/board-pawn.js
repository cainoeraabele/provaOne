...
var BOARD = function board_init(el, options) {
    ...
    function get_init_pos() {
        ...
        return "7k/pppppppp/8/8/8/8/PPPPPPPP/K7 w - - 0 1";
    ...
    function get_piece_img(piece) {
        return "url(\"" + encodeURI("/images/pieces/" + board.theme + "/" + piece.color + piece.type + (board.theme_ext || ".svg")) + "\")";
    }
...
//        alert(uci);
    ...
    function set_fen_selection() {
        var fen = document.getElementById("fen_selection").value;
    ...
    function set_fen(fen) {
        if(!fen) var fen = document.getElementById("fen_input").value;
    ...
    function change_theme_color() {
        ...
        if( board.theme_color ) {
            ...
            squares.forEach(function oneach(file, y) {
                file.forEach(function oneach(sq, x) {
                    if (squares[y][x].classList.contains("light")) {
                        squares[y][x].classList.remove("light");
                        squares[y][x].classList.add("newLight");
                    } else {
                        squares[y][x].classList.remove("dark");
                        squares[y][x].classList.add("newDark");
                    }
                });
            });
            ...
            squares.forEach(function oneach(file, y) {
                file.forEach(function oneach(sq, x) {
                    if (squares[y][x].classList.contains("newLight")) {
                        squares[y][x].classList.remove("newLight");
                        squares[y][x].classList.add("light");
                    } else {
                        squares[y][x].classList.remove("newDark");
                        squares[y][x].classList.add("dark");
                    }
                });
            });
